// PiGallery2 Trash Extension: Move to trash
const path = require('path');
const fsp = require('fs').promises;
const sharp = require('sharp');
const UserRoles = {LimitedGuest: 1, Guest: 2, User: 3, Admin: 4, Developer: 5};
const TRASH_DIR = process.env.PG2_TRASH_DIR || '/app/data/trash';
...
const ensurePlaceholder = async (extension) => {
	...
	} catch { // create simple 600x600 black square (fast to generate)
		const size = 600;
		await sharp({
			create: {
				width: size,
...
module.exports.init = async (extension) => {
	...
	extension.ui.addMediaButton({
		...
		popup: {
			header: 'Trash',
			body: `File will be moved to ${TRASH_DIR}`,
	...
	logger.info('[trash-ext] Extension initialized (Trash + icon)');
};
